...
def exportar_pdf(datos: dict, anio: int, mes: int, filepath: str):
    ...
    elements.append(Paragraph(
        "M = Mañana 06-14  |  T = Tarde 14-22  |  N = Noche 22-06  |  F = Franco  |  ⚠ = Anomalía",
        s_style))

    # Encabezado: para cada equipo mostramos las 2 personas
    header = ["#", "Fecha", "Día"]
    for eq in EQUIPOS:
        tecs = datos["tecnicos"].get(eq, ["—", "—"])
        n0 = tecs[0] if tecs[0] else "—"
        n1 = tecs[1] if tecs[1] else "—"
        header.append(f"Eq {eq}  Turno\n{n0}\n{n1}")
    ...
    c_map = {
        "M": rl_colors.HexColor("#FFF9C4"),
        "T": rl_colors.HexColor("#FFE0B2"),
        "N": rl_colors.HexColor("#BBDEFB"),
        "F": rl_colors.HexColor("#C8E6C9"),
    }
    ...
    for d in range(1, dias_mes + 1):
        ...
        for col_idx, eq in enumerate(EQUIPOS, start=3):
            turno = get_turno_equipo(eq, fecha)
            a0 = get_anomalia_persona_fecha(datos, eq, 0, fecha)
            a1 = get_anomalia_persona_fecha(datos, eq, 1, fecha)
            tecs = datos["tecnicos"].get(eq, ["—","—"])
            ...
            if a0 or a1:
                ...
                col_colors[(row_idx, col_idx)] = c_map.get(turno, rl_colors.white)
...
class AppTurnos:
    ...
    def _build_tab_calendario(self):
        ...
        for k, label in [("M","Mañana 06-14"),("T","Tarde 14-22"),
                          ("N","Noche 22-06"),("F","Franco")]:
            tk.Label(ley, text=f"  {k}: {label}  ",
                     bg=TURNO_COLORS[k], fg=TURNO_FG[k],
    ...
    def actualizar_calendario(self):
        ...
        for eq_idx, eq in enumerate(EQUIPOS):
            ...
            lbl = tk.Label(self.cal_inner, text=txt_hdr,
                           bg=COL_HDR_BG, fg=COL_HDR_FG, font=FONT_HDR,
        ...
        for d in range(1, dias_mes + 1):
            ...
            # Celdas de equipos — cada una con su propio color
            for eq_idx, eq in enumerate(EQUIPOS):
                col   = eq_idx + 3
                turno = get_turno_equipo(eq, fecha)
                tecs  = tecs_por_eq[eq]
                a0    = get_anomalia_persona_fecha(self.datos, eq, 0, fecha)
                a1    = get_anomalia_persona_fecha(self.datos, eq, 1, fecha)
                ...
                if es_hoy:
                    ...
                    bg_cel = TURNO_COLORS[turno]
                    fg_cel = TURNO_FG[turno]
                ...
                linea_turno = {"M":"🌅 Mañana","T":"🌇 Tarde",
                               "N":"🌙 Noche","F":"🏖 Franco"}[turno]
                if hay_anom:
                    lineas = [f"[{turno}] {linea_turno.split()[1]}",
                              estado(tecs[0] if len(tecs)>0 else "", a0),
                              estado(tecs[1] if len(tecs)>1 else "", a1)]
                else:
                    lineas = [f"{linea_turno}"]
